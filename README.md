...
- determine which proxies are working and measure latency with the optional Rust `firex-native` companion app
...
FireX Proxy handles that with an optional Rust companion app called `firex-native`. It runs on the user's computer, tests candidate proxies directly, measures response time, stops when it finds enough working proxies, and reports progress back to the extension UI.

When FireX Native is not installed, the extension still works as a normal proxy switcher. It can add proxies, sync sources, connect, disconnect, and apply bypass rules. FireX Native is recommended because most public proxies do not stay usable for long.

Installation instructions are in the [`firex-native` installation guide](./docs/firex-native-installation.md).

FireX Native release metadata lives in [`native-manifest.json`](./native-manifest.json). `latestVersion` tells the extension that an update is available. `minimumVersion` is the oldest native app version the current extension should still trust; raise it when compatibility actually changes.

To sync the manifest from the Rust crate version:

```sh
make native-manifest
```

To release a new native app version:

```sh
make native-release VERSION=0.1.2
```

That command updates the Rust crate version, refreshes `Cargo.lock`, syncs `native-manifest.json`, commits those version files, creates a `native-v0.1.2` tag, and pushes the branch and tag. Branch pushes only run the lightweight manifest sync job. The native tag starts the installer builds and publishes the FireX Native release assets.
